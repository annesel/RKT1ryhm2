...
test_student_typed(c("x>30", "30<x"), "Kasuta võrdluste tegemisel märke `>` ja `<`.")


test_object("x5", undefined_msg = "Objekti `x5` pole!", incorrect_msg = "Kas tegid võrdluse kujul `x < 40` ?")
test_output_contains("x4", incorrect_msg = "Vektor `x4` pole välja prinditud!")
test_student_typed(c("x<40", "40>x"), "Kasuta võrdluste tegemisel märke `>` ja `<`.")
...
test_student_typed(c("x4 & x5", "!(!x4 | !x5)"), "Kas tegid tehte kujul  `x4 & x5` ?")
